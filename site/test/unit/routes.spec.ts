...
describe("resolveRoute", () => {
  it("resolves manifest paths in both languages, slash-insensitively", () => {
    ...
    expect(resolveRoute(manifest, "/blog/contract-test-post/")).toMatchObject({ kind: "post", lang: "en" });
  ...
  it("maps /category/{slug}/ onto the filtered posts index", () => {
    expect(resolveRoute(manifest, "/category/labor/")).toMatchObject({ kind: "posts_index", category: "labor", lang: "en" });
    expect(resolveRoute(manifest, "/es/category/labor/")).toMatchObject({ kind: "posts_index", category: "labor", lang: "es" });
  });
  ...
  it("treats ?s= as search wherever it lands", () => {
    ...
    expect(resolveRoute(manifest, "/es/", { s: ["x", null] })).toMatchObject({ kind: "search", lang: "es", search: "x" });
  });

  it("reads ?paged and ?category from the query", () => {
    expect(resolveRoute(manifest, "/blog/", { paged: "2", category: "mutual" })).toMatchObject({ page: 2, category: "mutual" });
    expect(resolveRoute(manifest, "/blog/", { paged: "junk" }).page).toBe(1);
  });

  it("falls through to not_found with the language of the prefix", () => {
    expect(resolveRoute(manifest, "/nope/")).toMatchObject({ kind: "not_found", route: null, lang: "en" });
