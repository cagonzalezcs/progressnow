...
**Reason**: Superseded by the v4 role-named palette (`Brand palette tokens (v4 values)`).
**Migration**: Replace `bg-brand-red`/`text-red` → `bg-brand`/`text-brand`, `text-orange` → `text-accent`, `bg-cream` → `bg-alt` or `bg-white`, `bg-green-dark` → `bg-brand`, `border-yellow` (hero CTA) → `border-brand-light`; aliases exist only during the change.

### Requirement: Brand token custom properties
**Reason**: Describes the deleted SCSS/webawesome layer (`_global.scss`, `--wa-color-brand-*`); tokens live in `tailwind.css`.
**Migration**: None — no consumer remains.

### Requirement: Global baseline styles
**Reason**: Tailwind preflight and `@layer base` in `tailwind.css` provide the reset; `.wrapper` no longer exists.
**Migration**: Use `mx-auto max-w-[…] px-…` utilities.

### Requirement: Button system
**Reason**: The `.btn`/`wa-button` SCSS system was deleted with the legacy tree; pills are Tailwind class recipes in Bowlby One.
**Migration**: Use the pill recipes (`rounded-full font-display …`) documented on the styleguide.
