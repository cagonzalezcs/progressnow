...
The v4 canvas is a system change (palette, body face, page ground, tones, photos, wordmark). Source facts used below come from the `.dc.html` files imported to `wp-content/themes/design_handoff_rgvdsa_vue/v4/` (gitignored; re-import recipe in memory `rgvdsa-claude-design-import`). Contrast (computed): white on brand 7.1:1; accent on white 5.4:1, on alt 4.9:1; deep on white 11.1:1; muted `#4A5568` on white 7.5:1; `#C3CBE2` on ink 10.6:1; light-blue dashed border on brand 4.2:1 (non-text ✓); white on `#A9C7FF` 1.7:1 — never text on the light band.
