...
- **Polylang stub for tests.** `tests/polylang-stub.php`, loaded by `bootstrap.php` only when `pll_current_language` is undefined, implements `pll_default_language`, `pll_languages_list`, `pll_current_language`, `pll_get_post_language`/`pll_get_post`/`pll_get_post_translations` (post meta `_progressnow_test_lang` / `_progressnow_test_translations`), `pll_get_term_language`/`pll_get_term`/`pll_get_term_translations` (an option-backed map — WorDBless has no term-meta store), `pll_home_url` (`/` or `/es/`), identity `pll__`/`pll_e`, and no-op `pll_register_string`/`pll_the_languages`. It is **unconfigured by default** (no languages, default `''`), so every existing suite and the contract fixtures behave exactly as without Polylang; a test opts in with `progressnow_test_pll_configure( array( 'en', 'es' ), 'en' )` and the per-test option reset clears it. It is deliberately minimal: enough for the cache, categories, payload, and REST paths, never a Polylang emulation.
- **Per-test guard reset.** PHPUnit is one long "request", so `bootstrap.php` hooks `progressnow_cache_reset_bump_guard()` on `switch_theme` (every suite's `set_up()` starts with `switch_theme()`; a hook added before the first test survives WorDBless's per-test hook restore). Tests that model several requests call the reset explicitly.
- **Fixtures keep their zone.** `tests/test-contracts.php` (and the SEO event test) set `timezone_string` to the zone the committed fixtures were generated with, so the gcal `ctz`/`dates` bytes — which the Nuxt and Next contract specs also read — stay stable; the theme itself carries no zone.
- **Next's JSON-LD parser accepts UTC instants.** `next-js/lib/json-ld.ts` derives event ISO dates from the gcal URL; an offset-based site now emits `…Z` dates without `ctz`, so the parser maps a trailing `Z` to `+00:00` instead of rejecting the date.
