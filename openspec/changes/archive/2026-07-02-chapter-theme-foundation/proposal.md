...
the chapter's old website (the previous site) is dead and the chapter has no web presence beyond social media. The repo's custom Timber + Vite theme (`legacytheme`) has build tooling in place but no substance: no post types, no front page, default starter header. This change builds the theme foundation, mimicking a peer chapter (a peer chapter site) in content structure and design style.
...
- Create a front page (`front-page.php` + `views/front-page.twig` + `views/ui/` partials) mirroring a peer chapter site sections: hero, about/mission, upcoming events, get-involved grid, newsletter/social CTA. Copy hardcoded in Twig (generated from public the chapter info); media as inline SVG/CSS placeholders.
...
- Establish Progress Now design tokens as CSS custom properties (red `#dd1111`, dark red `#7c0909`, cream `#fff5e5`, black/white), mapped onto webawesome `--wa-*` vars.
...
- `design-tokens`: Progress Now brand CSS custom properties, global styles, and button system integrated with webawesome theming.
...
- `wp-content/themes/legacytheme/src/StarterSite.php` — CPTs, taxonomy, meta, context, nav menus, cleanup
