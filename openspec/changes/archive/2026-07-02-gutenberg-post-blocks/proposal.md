...
- Register six ACF blocks (`legacy/person-quote`, `video`, `audio`, `document`, `event-embed`, `action-callout`) via `blocks/*/block.json` + `render.php` — no second JS toolchain.
...
- New serializer `legacy_blog_blocks_from_content()` (`parse_blocks` → `PostBlock[]`); `legacy_blog_map_blocks()` becomes a `has_blocks()` dispatcher retaining the legacy ACF path until migration is verified.
...
- `inc/blog.php` — new parser, dispatcher, prose coalescer (reuses `legacy_blog_kses_prose()` from `backend-consolidation`); later deletes `post_blocks` group + legacy path
