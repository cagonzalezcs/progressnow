...
Prose HTML SHALL pass through a `wp_kses` allowlist (`legacy_blog_kses_prose()`) at serialize time before reaching any island `v-html` sink; attribute-bearing text fields SHALL be stripped or `br`-only.
...
Read minutes SHALL be computed once on `save_post_post` into `_legacy_read_minutes`; list serialization SHALL NOT load the flexible-content field per card.
...
The shared query builder `legacy_blog_posts_query()` SHALL prime author and thumbnail caches for its result set.
...
`legacy_cache_remember()` SHALL cache expensive payloads keyed to a content version option bumped on post/event/term/options saves.
