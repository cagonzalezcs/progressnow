...
- **Theme:** new `e2e/` (Playwright config, specs, axe baseline), `.wp-env.json`, `bin/seed-core.php` (ACF-free subset of `bin/seed.php`), `tests/integration/` (PHPUnit against MySQL, separate phpunit config), `package.json` scripts (`test:e2e`, `test:a11y`, `wp-env`).
