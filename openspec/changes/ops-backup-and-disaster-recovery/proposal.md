...
- **Restore drill:** performed on a staging host at least twice a year; the date and outcome recorded in `docs/backup-and-recovery.md`.
...
- **Optional automation:** `bin/backup.sh` (WP-CLI + rclone, env-configured, no secrets in the file) for hosts without snapshots; `.github/workflows` untouched.
...
- **Docs:** new `docs/backup-and-recovery.md`; pointers from `docs/deployment.md` §8 and `docs/incident-response.md` (owned by `security-detection-and-response`).
- **Scripts:** optional `wp-content/themes/progressnow/bin/backup.sh` + `restore-check.sh` (verify a dump imports and the site smoke-tests).
