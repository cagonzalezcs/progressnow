...
- **Shared configs at the root:** `.editorconfig`; one `prettier.config.mjs` referenced by each app; `tools/eslint/base.mjs` (TS + import hygiene + a11y baseline) extended by each app's flat config; one `vitest.shared.mts` (reporters, coverage provider, `passWithNoTests: false`). (planned)
...
- **Root:** `package.json`, `package-lock.json` (workspaces), `.editorconfig`, `prettier.config.mjs`, `tools/eslint/base.mjs`, `vitest.shared.mts`, `.npmrc`. (planned)
