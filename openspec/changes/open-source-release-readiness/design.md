...
- **One licence, MIT, at the root.** The owner chose MIT for the whole repo; the Timber starter is MIT (attribution kept in `composer.json` authors + README). The Nuxt app is not WP-derived, but a single licence keeps the project simple for adopters and contributors. Alternative — a different licence for `site/` — noted as an open question; easy to split later because `site/package.json` carries its own `license` field.
