...
The project is going to be open-sourced as a chapter-neutral organizing-site kit that anyone can adopt, with a public GitHub Pages demo. Today the repository cannot be published: the retired `legacytheme` is still tracked (25,381 files, mostly `node_modules`/`vendor`/a WorDBless WordPress copy), six plugins are vendored (3,324 files, including the commercially licensed ACF Pro and Polylang Pro), there is no LICENSE anywhere, and chapter identifiers and personal-data-shaped defaults survive in live code (`chapter.test` dev origin, the `legacy-a11y` storage key, the original chapter's copy embedded in `bin/scrub-brand.sh`, WhatsApp/garage defaults in the seed and fixtures, chapter names throughout the OpenSpec archive). Fixing this first shrinks the repo, speeds CI, and makes every later change publishable by default.

## What Changes

- **BREAKING (repo layout):** untrack and ignore `wp-content/themes/legacytheme/`, all of `wp-content/plugins/`, and `wp-content/backups-dup-lite/`. Plugins are never vendored again; the public repo ships only the theme, the Nuxt site, docs, infra reference, workflows, and OpenSpec.
...
- **Licensing + community files:** root `LICENSE` (MIT) and matching `license` fields in `style.css`, `composer.json`, and both `package.json`; root `README.md`, `CONTRIBUTING.md`, `CODE_OF_CONDUCT.md`, `SECURITY.md`; theme `package.json` renamed from `vite-project`. (planned)
- **Identifier + PII scrub:** neutral dev origin (`chapter.test`) in `nuxt.config.ts`, `site/.env.example`, `docs/deployment.md`; drop the `legacy-a11y` legacy-key migration in both `useA11ySettings.ts` copies; delete `bin/scrub-brand.sh` (the one-time migration has run; it embeds the original chapter's copy); replace messaging-platform and private-address defaults in `bin/seed.php` (EN + ES), `tests/fixtures/*.json`, `tests/test-pages.php`, and both `lib/fixtures/index.ts`; rewrite chapter names in the OpenSpec archive to neutral wording.
...
- **Public-fork release checklist** (`docs/open-source-release.md`): hygiene pass, fresh git history at fork, version tag, GitHub security advisories + branch protection. Documented here, executed at fork time. (planned)
...
- **Repo:** `git rm --cached` of ~28,700 files; `.gitignore` gains plugin/legacy/artifact rules; new root files `LICENSE`, `README.md`, `CONTRIBUTING.md`, `CODE_OF_CONDUCT.md`, `SECURITY.md`, `docs/open-source-release.md`; `bin/scrub-brand.sh` deleted. (planned)
...
- **CI:** `.github/workflows/ci.yml` new `hygiene` job + `.github/scripts/repo-hygiene.mjs`. (planned)
...
- **Local dev:** the owner's `.env` and MAMP host stay `chapter.test` (gitignored); untracked plugin and legacy-theme directories remain on disk until the owner deletes them.
