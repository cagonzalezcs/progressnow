...
- **Per-process internal token.** `crypto.randomBytes(32)` at module init in `lib/request-path.ts`; `renderInternally` sends `x-internal-token`; the proxy strips the four internal headers unless the token matches (`timingSafeEqual`). Multi-instance/edge: each instance validates its own token; if the loop crosses instances (Vercel), the header strip still applies because the token will not match — so the internal render falls back to the plain path, which still yields correct HTML (status is set by the outer response). Rationale: portable, no shared secret, no config.
