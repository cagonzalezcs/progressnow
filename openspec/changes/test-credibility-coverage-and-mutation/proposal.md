...
- **Tautology audit:** a one-time pass listing tests that assert code against itself (snapshot-only, fixture-only, or `expect(fn()).toEqual(fn())` patterns) and converting each to a behavior assertion or an explicit contract fixture; findings recorded in `docs/testing.md`.
- **Test taxonomy doc** (`docs/testing.md`): unit / contract / integration / e2e / a11y / mutation, what each proves, where it runs, how to add one.
...
- **JS apps:** vitest configs (coverage), `stryker.config.mjs` in `next-js`, `package.json` scripts; CI steps.
- **Theme:** `phpunit.xml` coverage filter, `infection.json5`, `composer.json` require-dev (`infection/infection`), CI PHP job with `pcov`.
- **CI:** coverage artifacts + PR summary; `mutation.yml` weekly workflow; `fixtures-guard` step.
- **Docs:** `docs/testing.md`; README "Testing" points at it; CONTRIBUTING (when it exists) references the fixture rule.
