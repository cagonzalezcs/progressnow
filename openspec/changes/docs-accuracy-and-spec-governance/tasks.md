...
- [x] 3.3 Run in warn mode for one merge, then fail; fix every finding (expect several in the theme README and archived proposals — archive is out of scope) — every finding fixed locally (stale archive links in the deployment guide and three READMEs, a not-yet-existing `docs/` path, 32 proposal lines marked `(planned)` across eight changes, an allowlist for runtime/removed/namespace tokens); the full-tree run is clean, so the job ships in fail mode (`--warn` remains on both scripts for a rollback)

## 4. OpenSpec configuration

- [x] 4.1 Fill `openspec/config.yaml` `context`: stack, layout, three frontends, shared-source rule, security invariants (hostile-input test per sink, `|raw` marker, no shared-source copies, no secrets in output, env-first settings)
- [x] 4.2 Fill `rules` for `proposal` (overlap statement, non-modification statement, "Impact" names tests), `design` (rejected alternative per decision), `tasks` (each requirement maps to a task naming its test), `specs` (WHEN/THEN scenarios only)
- [x] 4.3 Draft one small artifact with the CLI to confirm the context and rules are applied — `openspec new change scratch-governance-probe`, then `openspec instructions <proposal|design|specs|tasks> --json` returned the context and the artifact's rules for all four; scratch change deleted
