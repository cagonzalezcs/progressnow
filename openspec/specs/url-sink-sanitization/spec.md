# url-sink-sanitization Specification

## Purpose
Scheme allow-listing for every editable URL that reaches a front-end `href`/`src` binding through a `progressnow/v1` REST payload or an embedded island context, so no role can store a `javascript:`/`data:`/`vbscript:` link that a Vue, Nuxt or Next binding would later render. Complements `authoring-least-privilege` (who may store markup) and `block-serialization` (the block contracts that carry these fields). Implemented in `inc/sanitize.php` (`progressnow_safe_url()`), covered by `tests/test-url-sinks.php`, with a one-time content scan at `wp chapter audit-urls` (`inc/cli.php`).
