# template-output-escaping Specification

## Purpose
Context-correct output encoding for everything the WordPress theme prints: Twig autoescaping on by default (the theme's `esc_html` strategy, no double-encoding of kses-normalized storage), every `|raw` justified by a same-line `{# raw: kses|encoder|markup #}` marker, inline JSON (`__SHELL_DATA__`, JSON-LD, importmap) emitted only through the script-safe encoder, and a hostile-content suite that renders every public template. Complements `url-sink-sanitization` (URL schemes), `authoring-least-privilege` (what may be stored) and `structured-data` (the JSON-LD payloads). Implemented in `src/StarterSite.php`, `inc/escaping.php` (`progressnow_json_for_script()`), `bin/twig-audit.mjs` + `tests/test-twig-audit.php` (the gate) and `tests/test-output-escaping.php`.
