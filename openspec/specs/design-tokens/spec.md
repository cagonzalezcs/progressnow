...
Production artwork SHALL ship in `static/images/brand/` with semantic filenames: `cta-panel.svg` (the v4 blue panel, 1281×563, fills `#1848D8`/`#3E4480`/`#FFC800`/`#FFFFFF`), `star.svg`, `star-notch.svg`, `sparkle.svg` (fill `currentColor` so placement sets the color), `flames-tile-light.png` (2816×384, seamless `repeat-x`, consumed only as a CSS mask), `icon-twitter.svg` / `icon-instagram.svg` / `icon-facebook.svg`, `logo-square.svg`/`.png` and `share-default.jpg`. Photo placeholders (`hero-photo.jpg`, `who-photo.jpg`, `about-photo.jpg`) SHALL be neutral color photographs — the duotone is applied in CSS, never baked in. `logo-header.svg`, `logo-footer.svg`, `feature-art.svg`, `flames-tile.svg`, `flames-full.svg`, `hero-photo@2x.jpg` SHALL NOT ship. No regional artwork (county map, regional headline lockup, regional logo lockups) and no `social-icons.png` sprite SHALL ship. The Nuxt app SHALL reference brand artwork by URL from the `/site` payload so one copy in the theme serves both the PHP shell and the app. The brand README table SHALL list every file with its override field.
