...
The theme SHALL define the six canonical category slugs, labels, and colors once in `categories.json`, consumed by PHP (`progressnow_category_registry()`), TypeScript (JSON import), and verified against Tailwind `--color-cat-*` tokens by an automated test.
...
`progressnow_categories()` SHALL merge the WP term name and ACF term-meta `color` over the registry defaults when the term exists.
