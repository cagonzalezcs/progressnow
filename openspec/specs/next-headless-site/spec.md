# next-headless-site Specification

## Purpose
Contract for the Next.js headless frontend (`next-js/`): serving every public WordPress route at its own permalink from the `progressnow/v1` REST envelopes, fully server-rendered on first paint, kept fresh by push revalidation, with internal links re-homed onto the app origin and all chrome and copy driven by the `/site` envelope.
