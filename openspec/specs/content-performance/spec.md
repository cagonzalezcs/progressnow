# content-performance Specification

## Purpose
TBD - created by archiving change backend-consolidation. Update Purpose after archive.
## Requirements
### Requirement: Sanitized prose output
Prose HTML SHALL pass through a `wp_kses` allowlist (`progressnow_blog_kses_prose()`) at serialize time before reaching any island `v-html` sink; attribute-bearing text fields SHALL be stripped or `br`-only.
...
The shared query builder `progressnow_blog_posts_query()` SHALL prime author and thumbnail caches for its result set.
...
`progressnow_cache_remember()` SHALL cache expensive payloads keyed to a content version option bumped on post/event/term/options saves.
