...
export function Toc({ groups }: { groups: TocGroup[] }) {
  return (
    <nav
      ...
      data-tone="alt"
    >
      <p className="mb-4 text-sm font-extrabold uppercase tracking-[0.18em] text-accent">
        On this page
      </p>
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
        {groups.map((group) => (
          <div key={group.title}>
            <p className="mb-2 font-display text-[0.95rem] uppercase text-ink">{group.title}</p>
