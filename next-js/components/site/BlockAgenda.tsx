...
export function BlockAgenda({
  ...
  return (
    <div className="block-agenda flex w-full flex-col gap-2.5 md:gap-3">
      ...
      <ol className="m-0 flex list-none flex-col gap-[9px] p-0 md:gap-2.5">
        {items.map((item, i) => (
          <li
            key={i}
          ...
          >
            <span className="text-[0.95rem] font-extrabold text-brand md:text-base">
              {item.title}
            </span>
            <span className="text-[0.95rem] font-semibold md:text-base">{item.desc ?? ""}</span>
          </li>
        ))}
      </ol>
    </div>
