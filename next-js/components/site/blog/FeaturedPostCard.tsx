  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className="absolute left-3 top-3 rounded-full bg-brand px-3 py-1 text-[0.72rem] font-bold uppercase tracking-[0.06em] text-white lg:left-4 lg:top-4 lg:px-3.5 lg:py-[5px] lg:text-[0.75rem]">
          {featuredLabel}
        </span>
      </span>
      <span className="flex flex-col justify-center gap-2 px-5 pb-[22px] pt-[18px] lg:gap-3 lg:px-10 lg:py-9">
        <span className="text-[0.82rem] font-semibold text-muted lg:text-[0.85rem]">
          {post.date}
          {post.readMinutes ? ` · ${post.readMinutes} min read` : null}
        </span>
        <span className="font-display text-[1.15rem] leading-[1.25] [text-wrap:balance] lg:text-[clamp(1.3rem,2.4vw,1.7rem)] lg:leading-[1.2]">
          {post.title}
        </span>
        <span className="text-[0.95rem] leading-[1.55] text-muted lg:text-[1.05rem] lg:leading-[1.6]">
          {post.dek ?? post.excerpt}
        </span>
        <span className="mt-1 hidden items-center gap-3 text-[0.95rem] font-extrabold uppercase tracking-[0.03em] text-accent lg:flex">
          {readLabel}
