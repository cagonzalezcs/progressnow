...
**Opt in when the stand-in is short.** The whole-route boundary shows nothing, so it always qualifies. The blog archive's `h-40` skeleton is ~290px short of its results and moved the footer 372px, so it qualifies. `CalendarSkeleton` measured no movement at all (footer top 1340px before and after the fragment landed), so wrapping it would hide the footer for a window in which nothing was going to move — a regression in the other direction. The rule is the measurement, generalized: opt in when content landing would move the footer.
