...
**Opt in when a client navigation would move the footer.** Two conditions, both found by measurement. _Short stand-in:_ the whole-route boundary shows nothing, and the blog archive's `h-40` skeleton is ~290px short of its results and moved the footer 372px. `CalendarSkeleton` sat at 1178px against a grid that settles at 1336px. The condition is still not decorative: a skeleton that already holds its content's space must stay unwrapped, or the footer is hidden for a window in which nothing was going to move — a regression in the other direction.
...
**The calendar was re-measured, not assumed.** An earlier pass left `RouteCalendar` unwrapped: its boundary gated only `searchParams`, the events envelope was awaited above it, and the footer did not move. Main has since moved that fetch inside the boundary (`CalendarBody`), so the skeleton now stands in for data rather than for a query parse, and the footer sits at 1178px behind it before settling at 1336px. It opts in. The comment says why, because the condition is about where the data is awaited — not about which route it is.
