...
- **Animate only on a change of pathname.** React was transitioning on every update inside the boundary, which meant two extra full-page cross-fades: one per keystroke of the archive search, and one when a route's content resolved behind its skeleton — the page appearing to reload itself after it had already arrived. Design D6 already said URL-state updates "do not trigger a transition"; this makes it true. Reduce motion suppressed both, which is why the blog looked correct with it on and wrong with it off.
...
- `components/nav/RouteTransition.tsx` — the boundary animates only when the pathname changes; the settled path is state so a Suspense reveal re-renders it.
