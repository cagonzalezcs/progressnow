...
A chapter-neutral organizing-site kit for a **Progress Now** chapter: a WordPress theme that owns content, SEO and editing, plus a Nuxt 4 app that renders the public site as static files. Everything that names or pictures a chapter is data in wp-admin; everything shipped in the repo is a generic placeholder.
...
- **Two frontend modes** via `CHAPTER_FRONTEND`: `nuxt` (above) or the legacy `islands` mode (Vite-built Vue islands mounted on Twig, still in the theme until the cleanup phase). Rollback is a one-constant flip.
...
│   ├── src/            shared Vue/TS/Tailwind source (copied into site/app — see Design system)
...
├── site/                Nuxt 4 static rendition (see site/README.md)
...
define( 'CHAPTER_STATIC_DIR', ABSPATH . 'site/.output/public' );
```
...
**Shared source rule.** `src/components/site/**`, `src/components/ui/**`, `useA11ySettings.ts`, `schemas.ts` and `tailwind.css` in the theme are copied verbatim into `site/app/`. Edit the theme copy and re-copy; `site/test/unit/shared-source-drift.test.ts` fails on any drift. Twig partials and route components must keep the same class literals as their Vue twins.
